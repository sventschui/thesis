...
Die feature-detection in Texterkennungssoftware wird immer mehr mit künstlicher Intelligenz ersetzt. \textcite{Neuberg2017CreatingLearning} beschreibt wie Dropbox künstliche Intelligenz anwendet, um Texte aus Photographien von Dokumenten durchsuchbar zu machen. Zur Anwendung kommen dabei verschiedene Techniken aus dem Bereich der künstlichen Intelligenz: \textit{Long-Short-Term-Memory} (LSTM) Netzwerke, \textit{Connectionist Temporal Classification} (CTC), \textit{Convolutional Neural Network} (CNNs) und mehr~\autocite{Neuberg2017CreatingLearning}.
...
LSTM Modelle wurden im Bereich des \textit{Natural Language Processing} (kurz NLP)\todo[color=red,inline]{Begriff NLP erklären} bereits erfolgreich angewendet um Rechtschreibung und Grammatik zu korrigieren. So beschreibt \textcite{Weiss2016DeepSpelling} in seinem Blog, wie mit einem einfachen Neuronalen Netzwerk, bestehend aus nur 4 LSTM und 4 Dropout Schichten, bereits erfolgreich Rechtschreibfehler korrigiert werden können. 
